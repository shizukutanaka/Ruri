...
- 規模感: `src/` **39,418行**、高速テスト **2,921件**(`npm test`、**約8秒**)。
...
- **カバレッジゲートは修正済み・合否判定に使える**(2026-07)。閾値 95/90/98/95
  (lines/branches/functions/statements)に対し実測 **98.54 / 93.64 / 100 / 97.99**。
...
- **機械生成コードを再導入しない**。`presets.ts` も 2,206→32 宣言(38,695→702行)。
  `src/` 全体は 186,604→**39,418行**。`adapters/wav.ts` も 57→16 export(2,284→439行)。
