...
- **カバレッジゲートは修正済み・合否判定に使える**(2026-07)。閾値 95/90/98/95
  (lines/branches/functions/statements)に対し実測 **98.18 / 94.58 / 100 / 98.18**。
  かつて「80%閾値 vs 実測35.8%」と乖離していたのは**測る対象が誤っていた**ため —
  12.8万行の機械生成実装(`scale.ts`/`presets.ts`)を分母に含めていた。これらは
  公開APIでもなく専用スイートも削除済みなので coverage.exclude に入れた。
  **閾値を下げたのではなく、正しいものを測るようにした**(手書きコードは元から98%)。
